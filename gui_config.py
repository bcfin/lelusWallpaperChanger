...
class WallpaperConfigGUI:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.logger = get_logger('gui_config')
    ...
    def _create_advanced_tab(self) -> None:
        ...
        # Load current API keys using config manager (with fallback to .env)
        current_wallhaven_key = self.config_manager.get_api_key('wallhaven') or ""
        current_pexels_key = self.config_manager.get_api_key('pexels') or ""
        current_openweather_key = self.config_manager.get_api_key('openweather') or ""

        # Fallback to .env file if config manager doesn't find keys
        if not current_wallhaven_key or not current_pexels_key or not current_openweather_key:
            env_path = Path(__file__).parent / '.env'
            if env_path.exists():
                with open(env_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        if line.startswith('WALLHAVEN_API_KEY=') and not current_wallhaven_key:
                            current_wallhaven_key = line.split('=', 1)[1].strip()
                        elif line.startswith('PEXELS_API_KEY=') and not current_pexels_key:
                            current_pexels_key = line.split('=', 1)[1].strip()
                        elif line.startswith('OPENWEATHER_API_KEY=') and not current_openweather_key:
    ...
    def _save_api_keys(self) -> None:
        """Save API keys to both .env file and JSON config"""
        try:
...
"""
            ...
            if wallhaven_key:
                self.config_manager.update_json_value('api_keys.wallhaven.key', wallhaven_key)
            if pexels_key:
                self.config_manager.update_json_value('api_keys.pexels.key', pexels_key)
            if openweather_key:
                self.config_manager.update_json_value('api_keys.openweather.key', openweather_key)

            messagebox.showinfo("Success", "API Keys saved successfully!\n\nSaved to both .env file and config.json:\n• Wallhaven API Key\n• Pexels API Key\n• OpenWeatherMap API Key")
        except Exception as e:
