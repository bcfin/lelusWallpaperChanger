...
pip install --quiet pillow requests customtkinter matplotlib colorthief imagehash screeninfo scipy
...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%START_MENU%\Start Background Service.lnk'); $s.TargetPath = 'wscript.exe'; $s.Arguments = '\"%SCRIPT_DIR%run_hidden.vbs\"'; $s.WorkingDirectory = '%SCRIPT_DIR%'; $s.IconLocation = 'shell32.dll,25'; $s.Description = 'Start Wallpaper Changer Background Service'; $s.Save()"
...
if errorlevel 2 (
    ...
    echo Setting up autostart with robust path handling...
    
    REM Check if autostart setup script exists
    if not exist "%SCRIPT_DIR%setup_autostart.vbs" (
        echo [ERROR] setup_autostart.vbs not found!
        echo Autostart setup failed.
    ) else (
        REM Run the autostart setup script
        cscript.exe //nologo "%SCRIPT_DIR%setup_autostart.vbs"
        if errorlevel 1 (
            echo [ERROR] Autostart setup failed
            echo Check autostart_setup.log for details
        ) else (
            echo [OK] Autostart configured successfully
            echo   - Registry entry added
            echo   - Startup shortcut created
            echo.
            echo To disable autostart later, run: uninstall_autostart.vbs
        )
    )
)
