...
REM Remove autostart entries using robust script
echo Removing autostart entries...
if exist "%~dp0uninstall_autostart.vbs" (
    cscript.exe //nologo "%~dp0uninstall_autostart.vbs"
) else (
    REM Fallback to manual removal
    del "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\Wallpaper Changer.lnk" 2>nul
